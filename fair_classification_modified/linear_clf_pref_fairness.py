    ...
    def fit(self, X, y, x_sensitive, cons_params=None):
        ...
        if self.train_multiple == True:
            ...
            for k in set(x_sensitive):
                self.w[k] = Variable((X.shape[1],1)) # this is the weight vector
                self.w[k].value = [np.random.rand(X.shape[1])] # initialize the value of w -- uniform distribution over [0,1]
        else:
            self.w = Variable((X.shape[1],1)) # this is the weight vector
        ...
        if self.train_multiple == True:
            ...
            for k in set(x_sensitive):
                ...
                X_k = X[idx]
                y_k = y[idx]
                obj += sum_squares(self.w[k][1:]) * self.lam[k] # first term in w is the intercept, so no need to regularize that

                if self.loss_function == "logreg":
                    obj += sum(  logistic( multiply(-y_k, X_k@self.w[k]) )  ) / num_all # notice that we are dividing by the length of the whole dataset, and not just of this sensitive group. this way, the group that has more people contributes more to the loss
                    
                elif self.loss_function == "svm_linear":
                    obj += sum( maximum(0, 1 - multiply( y_k,  X_k@self.w[k])) ) / num_all
            ...
            obj = 0
            obj += sum_squares(self.w[1:]) * self.lam # regularizer -- first term in w is the intercept, so no need to regularize that
            if self.loss_function == "logreg":
                obj += sum(  logistic( multiply(-y, X@self.w) )  ) / num_all
            elif self.loss_function == "svm_linear":
                obj += sum( maximum(0, 1 - multiply( y,  X@self.w)) ) / num_all
            else:
                raise Exception("Invalid loss function")
        ...
        if cons_params is not None:
            ...
            if cons_type == -1: # no constraint
                ...
                constraints += self.get_di_cons_single_boundary(X, y, x_sensitive, self.w, cov_thresh)
            elif cons_type in [1,3]: # preferred imp, pref imp + pref treat
                constraints += self.get_preferred_cons(X, x_sensitive, self.w, cons_type, cons_params["s_val_to_cons_sum"])
            elif cons_type == 2:
                constraints += self.get_preferred_cons(X, x_sensitive, self.w, cons_type)
            else:
        ...
        if self.train_multiple == True:
            self.w = {}
            for k in set(x_sensitive):
                self.w[k] = np.array(self.w[k].value).flatten() # flatten converts it to a 1d array
        else:
            self.w = np.array(self.w.value).flatten() # flatten converts it to a 1d array
